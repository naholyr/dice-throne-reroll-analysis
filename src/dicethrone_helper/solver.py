...
class ExactSolver:
    ...
    def solve(self, ability: Ability, state: DiceState, rerolls: int) -> Policy:
        ...
        if is_success(self.character, ability, state):
            result = Policy(Fraction(1), Fraction(0), empty_timeline, (), True)
        elif rerolls == 0:
            result = Policy(Fraction(0), Fraction(0), (), (), False)
        else:
    ...
    def _best_policy(self, ability: Ability, state: DiceState, rerolls: int) -> Policy:
        candidates: list[tuple[Action, Fraction, Fraction, tuple[Fraction, ...]]] = []
        ...
        best_expected_rolls = min(candidate[2] for candidate in probability_ties)
        ties = [
            candidate
            for candidate in probability_ties
            if candidate[2] == best_expected_rolls
        ]
